...
const BOT_API_URL    = 'http://localhost:3001'; // Ganti ke IP/domain server bot kamu, misal: 'http://103.xxx.xxx.xxx:3001'
const BOT_API_SECRET = 'alip-ai'; // sama dengan global.botSecretKey di settings.js

// =============================================
// WA SETTINGS (Bot WA langsung)
// =============================================
...
function openWaSettings() {
  ...
  title.textContent = '⚙️ Pengaturan Nomor WA';
  body.innerHTML = `
    <div class="feat-label">NOMOR WA TUJUAN (stiker dikirim ke sini)</div>
    ...
    <button class="feat-btn" onclick="saveWaSettingsFromModal()">💾 Simpan</button>
...
function saveWaSettingsFromModal() {
  ...
  if (!phone) {
    status.textContent = '⚠️ Isi nomor WA dulu!';
    status.className = 'feat-status error';
    return;
  }
  saveWaSettings(phone);
  status.textContent = '✅ Tersimpan! Stiker akan dikirim ke ' + phone.replace(/\D/g, '');
  status.className = 'feat-status';
}

// ── Kirim stiker ke WA via Bot langsung ──
async function sendStickerToWA(blobOrUrl, statusEl) {
  const s = getWaSettings();
  if (!s.phone) {
    return { ok: false, msg: '⚙️ <a href="#" onclick="openWaSettings();return false;" style="color:#7c3aed;">Atur nomor WA tujuan</a> dulu di menu ⋮' };
  }
  ...
  try {
    ...
    // Kalau blob → kirim file langsung, kalau string URL → fetch dulu jadi blob
    if (typeof blobOrUrl === 'string') {
      const res = await fetch(blobOrUrl);
      const blob = await res.blob();
      form.append('file', blob, 'sticker.webp');
    } else {
      form.append('file', blobOrUrl, 'sticker.webp');
    }

    form.append('target', s.phone.replace(/\D/g, ''));
...
function openFeature(type) {
  ...
  if (type === 'brat') {
    ...
    const waNote = ws.phone
      ? `<div style="font-size:11px;color:#22c55e;margin-bottom:12px;">✅ WA terhubung · stiker dikirim ke <b>${ws.phone}</b></div>`
      : `<div style="font-size:11px;color:var(--muted);margin-bottom:12px;">⚙️ Belum atur nomor WA? <a href="#" onclick="openWaSettings();return false;" style="color:#7c3aed;">Atur sekarang</a> biar stiker langsung ke WA.</div>`;
...
async function runSticker() {
  ...
  if (botApiOnline && s.phone) {
    ...
    console.info('[Sticker] Bot API offline, pakai API konversi');
  }

  // ══ MODE 2: Fallback — konversi via API eksternal ══
  ...
  try {
    ...
    // Auto kirim via Bot WA jika nomor sudah diset
    if (s.phone) sendStickerFromResult();
